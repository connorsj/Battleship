...
public class Client
{
	...
	{
		for(int x=0; x < board.myShips.size(); x++){
			if(board.myShips.get(x).isAlive())
				return false;
		}
		return true;
	}
	...
	{
		return this.man.getOpponent(this).allEnemyShipsAreDestroyed();
	}
	...
	boolean processCommand() throws IOException
	{
		...
		String[] s = line.split(" ");//removes spaces
		String input = s[0]; //gets input character
		
		if(input == "F")
		{
			int x = Integer.parseInt(s[1]); //cast string coordinates to ints
			int y = Integer.parseInt(s[2]);
			if(x < 0 || y < 0 || x >=10 || y >= 10){//checking bounds of coordinates
				return false;
			}
			String[] fireCmd = null;
			fireCmd[0] = s[1];
			fireCmd[1] = s[2];
			if(processFireCmd(fireCmd) == false){
				return false;
			}				
		}
		if(input == "C"){
	...
	boolean processChatCmd( String s )
	{
		...
		if(this.man.getOpponent(this) == null){ //no opponent so return false
			return false;
		}
		else{ //opponent so print what they say
			this.man.getOpponent(this).out.print(this.name + ":" + s);
			return true;
		}
